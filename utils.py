...
class EarlyStopping:
    def __init__(self, patience=20,metric="loss",min_delta=0):
    ...
    def __call__(self, val_f1):
        if self.metric in ["rpc","auc","f1"]:
            if self.best_f1 is None:
                self.best_f1 = val_f1
            elif val_f1<= self.best_f1 - self.min_delta:
                self.counter += 1
                print("early_stop:" + str(self.counter) + " out of patience " +str(self.patience))
...
def load_opt(model, config, logging):
    ...
    # Reinitialize the optimizer with only the trainable parameters
...
def prepare_tensorboard(result_path):
    ...
    val_path = os.path.join(result_path, ''
                                         'valid')
