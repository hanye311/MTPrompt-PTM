...
class ESM2(nn.Module):
    ...
    # def forward(self, tokens, ptm_embeddings,task_ids = None, repr_layers=[], need_head_weights=False, return_contacts=False):
    def forward(self, tokens,padding_masks=None,task_ids=None, configs=None,repr_layers=[], need_head_weights=False,
        ...
        if padding_masks==None:
            padding_mask = tokens.eq(self.padding_idx)  # B, T
        x = self.embed_scale * self.embed_tokens(tokens)
        # x=x+ptm_embeddings   #todo remove if without ptm_embeddings
        ...
        #"""
        if configs.encoder.prompt.if_attention_masks==True:
            ...
            if self.prefix_module.prompt_len != 0:
                attention_mask[:, :self.prefix_module.prompt_len, :self.prefix_module.prompt_len] = True
        else:
            attention_mask=None
        ...
        if self.prefix_module is not None:    #todo
            if not self.prefix_module.if_pass_to_MHA:
                # remove the prompt_tokens
